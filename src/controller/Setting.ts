...
export default class Setting {
        ...
        for (const llm of llmList) {
            if (llm.selected) {
                if (
                    ...
                    resultList.push("Llm - Url: Can only contain a standard url format without the final slash.");
                }
    ...
    private tableInsert = async (mcpSessionId: string, id: number, llm: modelSetting.Illm[], isDelete: boolean): Promise<boolean> => {
        let isResult = false;

        if (mcpSessionId !== "") {
            isResult = await database.pool
                .query(`INSERT INTO "${mcpSessionId}_setting" (id, llm, delete) VALUES ($1, $2::jsonb, $3) ON CONFLICT DO NOTHING;`, [
                    id,
                    JSON.stringify(llm),
                    isDelete
                ])
    ...
    tableCreate = async (mcpSessionId: string): Promise<boolean> => {
        ...
        if (mcpSessionId !== "") {
            const isQuery = await database.pool
                .query(
                    `CREATE TABLE IF NOT EXISTS "${mcpSessionId}_setting" (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, llm JSONB NOT NULL, delete BOOLEAN NOT NULL);`
                )
                .then(() => {
                    return true;
                })
